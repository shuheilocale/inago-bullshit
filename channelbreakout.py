...
class ChannelBreakOut:
    def __init__(self, key, sec, line_token):
        ...
        self._cost = 3000
        self.order = Order(key, sec)
        self.api = pybitflyer.API(key, sec)
    ...
    def describeResult(self, entryTerm, closeTerm, fileName=None, candleTerm=None, rangeTh=5000, rangeTerm=15, originalWaitTerm=10, waitTh=10000, showFigure=True, cost=0):
        ...
        if fileName == None:
            ...
            start_timestamp = datetime.datetime(2017, 2, 20, s_hour, s_min, 0, 0).timestamp()
            end_timestamp = datetime.datetime(2020, 3, 31, e_hour, e_min, 0, 0).timestamp()
...
def optimization(key, sec, line_token):
    ...
    for i in entryAndCloseTerm:
        for j in rangeThAndrangeTerm:
            for k in waitTermAndwaitTh:
                channelBreakOut = ChannelBreakOut(key, sec, line_token)
...
if __name__ == '__main__':
    ...
    #最適化
    #optimization(key, sec, line_token)